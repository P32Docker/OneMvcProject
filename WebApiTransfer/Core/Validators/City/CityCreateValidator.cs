		...
		RuleFor(x => x.CountryId)
			...
			.DependentRules(() =>
			{
				RuleFor(x => x.CountryId)
					...
					.WithMessage("Данна країна не існує");
			});

		RuleFor(x => x.Slug)
			.NotEmpty().WithMessage("Slug міста не може бути порожнім")
			.MaximumLength(100).WithMessage("Slug міста не може перевищувати 100 символів")
			.DependentRules(() =>
			{
				RuleFor(x => x.Slug)
